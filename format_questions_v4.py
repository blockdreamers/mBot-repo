...
def format_questions(file_path):
    """문제를 깔끔하게 포맷팅합니다 (v4.0 - 질문 패턴 첫 줄 버그 수정)"""
    ...
    for idx, (start_line, question_num) in enumerate(question_starts):
        ...
        # 특정 문제들 미리보기 (157번 포함)
        if question_num in [141, 142, 157, 288, 289] or idx < 3 or idx >= len(question_starts) - 3:
...
def format_single_question(question_num, lines):
    """개별 문제를 포맷팅합니다 (v4.0 - 질문 패턴 분리 개선)"""
    ...
    if choice_starts:
        ...
        # 보기가 없는 경우 (전체가 본문)
        main_content_lines = cleaned_lines
        choices = {}
    ...
    for pattern in question_patterns:
        ...
        if matches:
            ...
            # 특별 처리: 질문이 첫 번째 줄에 있는 경우
            if match.start() < 100:  # 질문이 앞쪽에 있다면
                # 질문 이후의 부분을 스토리로 처리
                after_question = main_content[match.end():].strip()
                if after_question:
                    story_part = after_question
                    print(f"  문제 {question_num}: 질문이 앞쪽에 위치, 이후 텍스트를 스토리로 처리")
                    print(f"  문제 {question_num}: 스토리 '{story_part[:100]}...'")
                else:
                    story_part = ""
            else:
                # 일반적인 경우: 질문 이전의 부분을 스토리로 처리
...
def main():
    print("=" * 60)
    print("CR 문제 포맷팅 스크립트 v4.0 (질문 패턴 첫 줄 버그 수정)")
    print("=" * 60)
    ...
    if format_questions(file_path):
        ...
        # 157번 문제 특별 확인
        match_157 = re.search(r'^157\. (.+?)(?=158\.|$)', content, re.MULTILINE | re.DOTALL)
        if match_157:
            problem_157 = match_157.group(1)[:200]
            if "European wild deer" in problem_157:
                print("✅ 157번 문제 본문이 올바르게 복원되었습니다!")
                print(f"   시작: '{problem_157[:100]}...'")
            else:
                print("❌ 157번 문제 본문이 여전히 누락되었습니다.")
