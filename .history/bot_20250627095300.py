...
def handle_button(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        print(f"✅ User {user_id} answered Q{qn} -> {'Correct' if is_correct else 'Wrong'} ({chr(64+selected)}) in {duration:.1f}s — saved to DB")

    except Exception as e:
        print(f"❌ DB Save Failed: User {user_id} | Q{qn} | Error: {str(e)}")
