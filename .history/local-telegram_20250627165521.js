...
bot.hears(/^\/q(\d*)$/, async (ctx) => {
  ...
  const questions = (await getAllQuestions()).filter((q) => q.type.toLowerCase() === currentSubject.toLowerCase());
...
bot.on("callback_query", async (ctx) => {
  const [qid, selectedStr, startStr, subject] = ctx.callbackQuery.data.split("|"); // ✅ UUID 그대로 사용
  ...
  const q = questions.find((q) => q.id === qid); // ✅ UUID 비교
