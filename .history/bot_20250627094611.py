...
def send_question(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        answered = supabase.table("user_answers") \
            ...
            .eq("user_id", user_id) \
            .execute()
        answered_ids = {str(row["question_id"]) for row in answered.data}

        # 전체 문제 중 아직 안 푼 문제 찾기
        ...
        for q in questions.data:
            ...
            print(f"✅ User {user_id} has solved all available questions.")
            return
...
def handle_button(update: Update, context: CallbackContext) -> None:
    ...
    answered_ids = {str(row["question_id"]) for row in answered}
