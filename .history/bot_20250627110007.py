...
def start(update: Update, context: CallbackContext) -> None:
    keyboard = [["/q", "/wrong", "/stats"], ["/q1", "/q2", "/q3"]]
    reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
    update.message.reply_text("안녕하세요! 아래 버튼 중 하나를 눌러 시작해보세요 👇", reply_markup=reply_markup)
...
def handle_button(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        print(f"⚠️ Failed to calculate progress: {str(e)}")
...
def wrong_answers(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        if not rows:
            ...
            return
        ids = [
            f"문제 {supabase.table('questions') \
                .select('question_number') \
                .eq('id', row['question_id']) \
                .execute().data[0]['question_number']}"
            for row in rows
        ]
        update.message.reply_text("❌ 당신이 틀린 문제 목록:\n" + "\n".join(ids))
    except Exception as e:
...
def main():
    ...
    dp.add_handler(MessageHandler(Filters.text & Filters.regex(r"^/q\d+$"), send_question))
